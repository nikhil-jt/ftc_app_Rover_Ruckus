...
public abstract class BaseAutonomous extends LinearOpMode {
    ...
    protected DcMotor left2 = null; //Inits the object to null
    protected DcMotor right = null; //Inits the object to null
    protected DcMotor right2 = null; //Inits the object to null
    ...
    protected VuforiaTrackable initAndWaitForStart() {
        ...
        left = hardwareMap.dcMotor.get("fl"); //Assign values to motors, servos, and the color sensor
        left2 = hardwareMap.dcMotor.get("bl"); //Assign values to motors, servos, and the color sensor
        right = hardwareMap.dcMotor.get("fr");
        right2 = hardwareMap.dcMotor.get("br");
        ...
        right2.setDirection(DcMotorSimple.Direction.REVERSE); //Reverses one motor or else they would be spinning opposite directions
        left.setDirection(DcMotorSimple.Direction.REVERSE); //Reverses one motor or else they would be spinning opposite directions
...
//        vuforiaTurn((VuforiaTrackableDefaultListener) relicTemplate.getListener(), 0);
    ...
    protected void encoderMove(double power, double leftInches, double rightInches, int timeoutS) {
        if (!isStopRequested()) {
            ...
            int newLeft2Target = left2.getCurrentPosition() + (int) (leftInches * COUNTS_PER_INCH);
            int newRightTarget = right.getCurrentPosition() + (int) (rightInches * COUNTS_PER_INCH);
            int newRight2Target = right2.getCurrentPosition() + (int) (rightInches * COUNTS_PER_INCH);
            ...
            left2.setPower(Math.abs(power));
            right.setPower(Math.abs(power));
            right2.setPower(Math.abs(power));
...
//        vuforiaMove((VuforiaTrackableDefaultListener) relicTemplate.getListener(), 2.35, 20);
        encoderMove(0.5, 2.3, 60);

        jewelservo.setPosition(0.88);
        sleep(700);
        jewelservo.setPosition(0.98);
        sleep(200);
...
//            sleep(1000);
            ...
            encoderMove(0.6, -rightLength, rightLength, 40);
